...
export default function Login() {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      const isAccountSwitch = Boolean(
        previousUserId && previousUserId !== data.userId,
      );
      // Clearing the whole cache on every login made tokens/saved analyses "disappear"
      // until refetches; only wipe when a different account signs in on this device.
      if (isAccountSwitch) {
        queryClient.clear();
      } else {
        queryClient.invalidateQueries({ queryKey: ["/api/user", data.userId] });
        queryClient.invalidateQueries({ queryKey: ["/api/analyses/saved", data.userId] });
        queryClient.invalidateQueries({ queryKey: ["/api/analyses", data.userId] });
        queryClient.invalidateQueries({ queryKey: ["/api/community/feed", data.userId] });
      }
      localStorage.setItem("userId", data.userId);
      localStorage.setItem("loginCompleted", "true");
      // Store user object for the rest of the app to use
  ...
  useEffect(() => {
    // Prevent double-injection which causes Phone.Email script to crash or reload
    let script = document.getElementById("pe-script") as HTMLScriptElement;
    if (!script) {
