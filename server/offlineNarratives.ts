...
const en = (c: OfflineNarrativeCtx): OfflineNarrativeBundle => ({
  ...
  newsHighlights: `Symbol-specific headlines need the live AI research service (Perplexity). It is unavailable right now—usually because the API key is missing/invalid, API quota is exhausted, or the provider returned an auth/billing error. The chart and price levels in this report still use market data for the symbol. For breaking news, use a trusted financial source.`,
