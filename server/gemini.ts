...
function getMockAnalysis(symbol: string, duration: string, language: string = "en"): MarketAnalysisResult {
  ...
  // Duration translations for each language
  const durationTranslations: Record<string, Record<string, string>> = {
    en: { long_term: "long term", short_term: "short term", scalping: "scalping" },
    hi: { long_term: "दीर्घकालिक", short_term: "अल्पकालिक", scalping: "स्कैल्पिंग" },
    es: { long_term: "largo plazo", short_term: "corto plazo", scalping: "scalping" },
    zh: { long_term: "长期", short_term: "短期", scalping: "剥头皮" },
    de: { long_term: "langfristig", short_term: "kurzfristig", scalping: "Scalping" },
    fr: { long_term: "long terme", short_term: "court terme", scalping: "scalping" },
    ar: { long_term: "طويل الأجل", short_term: "قصير الأجل", scalping: "المضاربة السريعة" },
    pt: { long_term: "longo prazo", short_term: "curto prazo", scalping: "scalping" },
    ru: { long_term: "долгосрочный", short_term: "краткосрочный", scalping: "скальпинг" },
    ja: { long_term: "長期", short_term: "短期", scalping: "スキャルピング" },
    ko: { long_term: "장기", short_term: "단기", scalping: "스캘핑" },
    it: { long_term: "lungo termine", short_term: "breve termine", scalping: "scalping" },
  };

  const translatedDuration = durationTranslations[language]?.[duration] || duration.replace("_", " ");
  ...
  const mockTexts: Record<string, any> = {
    en: {
      ...
      analysis: `Based on comprehensive analysis of all indicators, market sentiment, and technical factors, a ${isBullish ? "BUY" : "SELL"} position is recommended for ${symbol} in the ${translatedDuration} timeframe with ${confidence}% confidence.`,
    },
    hi: {
      ...
      analysis: `सभी संकेतकों, बाजार भावना और तकनीकी कारकों के व्यापक विश्लेषण के आधार पर, ${symbol} के लिए ${translatedDuration} समय सीमा में ${confidence}% विश्वास के साथ ${isBullish ? "खरीद" : "बिक्री"} की स्थिति की सिफारिश की जाती है।`,
    },
    es: {
      ...
      analysis: `Basado en el análisis exhaustivo de todos los indicadores, sentimiento del mercado y factores técnicos, se recomienda una posición de ${isBullish ? "COMPRA" : "VENTA"} para ${symbol} en el marco temporal ${translatedDuration} con ${confidence}% de confianza.`,
    },
    zh: {
      ...
      analysis: `基于对所有指标、市场情绪和技术因素的综合分析，建议在${translatedDuration}时间框架内对${symbol}采取${isBullish ? "买入" : "卖出"}仓位，置信度为${confidence}%。`,
    },
    de: {
      ...
      analysis: `Basierend auf umfassender Analyse aller Indikatoren, Marktstimmung und technischer Faktoren wird eine ${isBullish ? "KAUF" : "VERKAUF"}-Position für ${symbol} im ${translatedDuration} Zeitrahmen mit ${confidence}% Zuversicht empfohlen.`,
    },
    fr: {
      ...
      analysis: `Sur la base d'une analyse complète de tous les indicateurs, du sentiment du marché et des facteurs techniques, une position ${isBullish ? "D'ACHAT" : "DE VENTE"} est recommandée pour ${symbol} dans le cadre temporel ${translatedDuration} avec ${confidence}% de confiance.`,
    },
    ar: {
      ...
      analysis: `بناءً على التحليل الشامل لجميع المؤشرات ومعنويات السوق والعوامل الفنية، يوصى بموقف ${isBullish ? "شراء" : "بيع"} لـ ${symbol} في الإطار الزمني ${translatedDuration} بثقة ${confidence}%.`,
    },
    pt: {
      ...
      analysis: `Com base na análise abrangente de todos os indicadores, sentimento de mercado e fatores técnicos, uma posição de ${isBullish ? "COMPRA" : "VENDA"} é recomendada para ${symbol} no prazo ${translatedDuration} com ${confidence}% de confiança.`,
    },
    ru: {
      ...
      analysis: `На основе комплексного анализа всех индикаторов, рыночных настроений и технических факторов рекомендуется ${isBullish ? "ПОКУПКА" : "ПРОДАЖА"} для ${symbol} в ${translatedDuration} таймфрейме с ${confidence}% уверенностью.`,
    },
    ja: {
      ...
      analysis: `すべての指標、市場センチメント、テクニカル要因の包括的な分析に基づき、${symbol}について${translatedDuration}の時間枠で${confidence}%の信頼度で${isBullish ? "買い" : "売り"}ポジションが推奨されます。`,
    },
    ko: {
      ...
      analysis: `모든 지표, 시장 심리 및 기술적 요인에 대한 종합적인 분석을 바탕으로 ${symbol}에 대해 ${translatedDuration} 시간 프레임에서 ${confidence}% 신뢰도로 ${isBullish ? "매수" : "매도"} 포지션이 권장됩니다.`,
    },
    it: {
      ...
      analysis: `Sulla base dell'analisi completa di tutti gli indicatori, del sentimento di mercato e dei fattori tecnici, si raccomanda una posizione di ${isBullish ? "ACQUISTO" : "VENDITA"} per ${symbol} nel timeframe ${translatedDuration} con ${confidence}% di confidenza.`,
    },
  };
